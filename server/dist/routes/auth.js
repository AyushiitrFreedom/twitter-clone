...
export const authRouter = router({
    register: publicProcedure.query(async (opts) => {
        ...
        const newuser = { username: opts.ctx.req.body.username, email: opts.ctx.req.body.email, password: userPassword, id: uuidv4() };
    ...
    test: publicProcedure.query(async (opts) => {
        const result = await db.select().from(user).where(eq(user.email, "KJHIGF"));
